...
@Component({
  selector: 'app-faculty-accouncement-display',
  templateUrl: './faculty-accouncement-display.component.html',
  styleUrls: ['./faculty-accouncement-display.component.css']
})
export class FacultyAccouncementDisplayComponent implements OnInit {
  private makeAnnouncementForm = this.formBuilder.group({
  ...
  makeAnnouncement() {
    const formData = {
      courseKey: this.key,
      title: this.makeAnnouncementForm.value.title,
      details: this.makeAnnouncementForm.value.details,
    };
    const result = this.courseService.makeAnnouncements(formData);
